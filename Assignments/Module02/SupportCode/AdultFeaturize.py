...
class OneHotEncoder(object):
    def __init__(self, dataSet, featureIndex, featureName):
        ...
        self.names = ["is_%s_%s" % (self.featureName, value)
...
def GetMeanAndSD(dataSet, featureIndex):
    featureVals = list(map(lambda row: float(row[featureIndex]), dataSet))
    return (mean(featureVals), std(featureVals))
...
class AdultFeaturize(object):
    ...
    def __init__(self):
        ...
        self.featureNames = ['age', 'workclass', 'fnlwgt', 'education', 'education-num', 'marital-status', 'occupation',
                             'relationship', 'race', 'sex', 'capital-gain', 'capital-loss', 'hours-per-week', 'native-country']
        self.isNumeric = [True, False, True, False, True, False,
                          False, False, False, False, True, True, True, False]
        self.isCategorical = [False, True, False, True, False,
                              True, True, True, True, True, False, False, False, True]
        self.means = [None for _ in self.isNumeric]
        self.deviations = [None for _ in self.isNumeric]

        # Leave out some features by default
        self.includeFeature = [True, True, False, True, True,
    ...
    def CreateFeatureSet(self, xRaw, yRaw, useCategoricalFeatures=True, useNumericFeatures=False):
        self.useFeature = [self.includeFeature[i] and ((useCategoricalFeatures and self.isCategorical[i]) or (
            useNumericFeatures and self.isNumeric[i])) for i in range(len(self.featureNames))]

        self.oneHotEncoders = []
        for featureIndex in range(len(self.useFeature)):
            if self.useFeature[featureIndex] and self.isCategorical[featureIndex]:
                self.oneHotEncoders.append(OneHotEncoder(
                    xRaw, featureIndex, self.featureNames[featureIndex]))
            elif self.useFeature[featureIndex] and self.isNumeric[featureIndex]:
                # normalize
                (mean, deviation) = GetMeanAndSD(xRaw, featureIndex)
                self.means[featureIndex] = mean
                self.deviations[featureIndex] = deviation
    ...
    def _FeaturizeX(self, xRaw):
        ...
        for i in range(len(self.useFeature)):
            if self.useFeature[i]:
                if self.isNumeric[i]:
                    raw = float(xRaw[i])
                    normalized = (raw - self.means[i])/self.deviations[i]
                    featureVector.append(normalized)
                    # featureVector.append(float(xRaw[i]))
                else:
                    featureVector = featureVector + \
                        self.oneHotEncoders[i].GetOneHotEncodingFor(xRaw[i])
    ...
    def GetFeatureCount(self):
        if not self.featureSetCreated:
            raise UserWarning(
                "Trying to GetFeatureCount count before calling CreateFeatureSet")
    ...
    def GetFeatureInfo(self, index):
        if not self.featureSetCreated:
            raise UserWarning(
                "Trying to GetFeatureIndex before calling CreateFeatureSet")
            ...
            if self.useFeature[i]:
                if self.isNumeric[i]:
                    ...
                    nextIndex = currentIndex + \
                        self.oneHotEncoders[i].GetFeatureCount()
                    if index >= currentIndex and index < nextIndex:
                        return self.oneHotEncoders[i].GetNameForIndex(index - currentIndex)
                    else:
                        currentIndex = nextIndex

        raise UserWarning("Tried to get info for feature index %d but there are only %d features" % (
            index, self.GetFeatureCount()))
